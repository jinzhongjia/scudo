...
const ELF_program_header = packed struct {
    p_type: u32,
    p_offset: u32,
    p_vaddr: u32,
    p_paddr: u32,
...
pub fn load(elf_addr: usize) usize {
    ...
    const ph_tbl = @intToPtr([*]ELF_program_header, elf_addr + elf.e_phoff)[0..elf.e_phnum];
    ...
    for (ph_tbl) |ph| {
        ...
        if (ph.p_type == PT_LOAD) {
            ...
            while (addr < (ph.p_vaddr + ph.p_memsz)) : (addr += x86.constant.PAGE_SIZE) {
            ...
            @memcpy(dest, src[0..ph.p_filesz]);
            @memset((dest + ph.p_filesz)[0 .. ph.p_memsz - ph.p_filesz], 0);
        }
    }
