...
pub fn virtualToPhysical(v_addr: usize) ?usize {
    ...
    return x86.constant.pageBase(pt_entry.*);
}
...
pub fn map(v_addr: usize, p_addr: ?usize, flags: u32) void {
    ...
    if (v_addr < layout.IDENTITY) {
        tty.panic("map failed, v_add smaller than layout.IDENTITY", .{});
    }
    ...
    if (p_addr) |p| {
        ...
        pt_entry.* = x86.constant.pageBase(p) | flags | PAGE_PRESENT;
    } else {
        if (pt_entry.* & PAGE_ALLOCATED != 0) {
            // Reuse the existing allocated page.
            pt_entry.* = x86.constant.pageBase(pt_entry.*) | flags | PAGE_PRESENT | PAGE_ALLOCATED;
        } else {
...
pub fn unmap(v_addr: usize) void {
    if (v_addr < layout.IDENTITY) {
        tty.panic("unmap failed, v_add smaller than layout.IDENTITY", .{});
    }
    const pd_entry = pdEntry(v_addr);
...
fn pageFault() void {
    ...
    const address = x86.assembly.readCR2();
    // Get the error code from the interrupt stack.
    const code = interrupt.context.error_code;
...
pub fn initialize() void {
    ...
    if (pmem.stack_end > layout.IDENTITY) {
        tty.panic("stack_end > layout IDENTITY, stack_end: 0x{X}", .{pmem.stack_end});
    }
